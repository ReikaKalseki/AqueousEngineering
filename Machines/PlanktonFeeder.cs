...
namespace ReikaKalseki.AqueousEngineering {
	...
	public class PlanktonFeeder : CustomMachine<PlanktonFeederLogic>, MultiTexturePrefab {
		...
		public PlanktonFeeder(XMLLocale.LocaleEntry e) : base(e.key, e.name, e.desc, "bedc40fb-bd97-4b4d-a943-d39360c9c7bd") {
			this.addIngredient(TechType.FiberMesh, 1);
			this.addIngredient(TechType.Pipe, 5);
		...
		public override void initializeMachine(GameObject go) {
			...
			this.initializeStorageContainer(con, 6, 4);
	...
	public class PlanktonFeederLogic : CustomMachineLogic {
		...
		protected override void updateEntity(float seconds) {
			if (bubbles == null || bubbles.Length == 0) {
				bubbles = this.GetComponentsInChildren<ParticleSystem>();
			...
			if (Vector3.Distance(Player.main.transform.position, transform.position) >= PlanktonFeeder.RANGE)
				return;
			if (this.consumePower(PlanktonFeeder.POWER_COST * seconds) && storage.container.GetCount(PlanktonFeeder.fuel.TechType) > 0) {
				this.setState(true);
				float r = PlanktonFeeder.RANGE;
				bool any = false;
				WorldUtil.getGameObjectsNear(gameObject.transform.position, r, go => any |= this.tryBreedCycle(go, seconds));
		...
		private void setState(bool enable) {
			if (enable == active && this.getAge() >= 1)
				return;
			active = enable;
			foreach (ParticleSystem p in bubbles) {
				if (enable)
